	...
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<%-- <form action="User_list.action" method="post" class="form-search" style="display: inline;">
	          &nbsp;话题名称：
			  <input name="s_user.name" value="${s_user.name }" type="text" class="input-medium search-query" placeholder="输入话题名称..."/>
			  &nbsp;所属版块：
								...
								<c:forEach items="${userList }" var="user">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;width: 150px;">
											<c:choose>
					                  	  		<c:when test="${not empty user.sectionList && fn:length(user.sectionList)==0 && user.type!=2 }">
					                  	  			<font style="color: black;">普通用户</font>	
					                  	  		</c:when>
					                  	  		<c:when test="${not empty user.sectionList && fn:length(user.sectionList)!=0 && user.type!=2 }">
					                  	  			<font style="color: blue;">题主</font>
